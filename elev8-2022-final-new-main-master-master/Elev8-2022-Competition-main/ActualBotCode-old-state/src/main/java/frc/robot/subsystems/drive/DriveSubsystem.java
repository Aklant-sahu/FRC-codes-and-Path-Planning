...
// updating robot pose
    ...
    // getting angle in -180 to 180 range
  public double getPoseAngle() {
    return this.pose.getRotation().getDegrees();
  
  }

    // setting speeds when we want to drive
  ...
  // gettin average encoder distance from four motors.
  ...
  // gets dtheta when we want to travel over very short distances
  public double getDTheta() {
    return this.d_theta;
  }

  // need of this function as in this left and right side  motors would be spinning in opposite direction at the same value of velocity
  ...
  // setting encoder poition or counts to zero and also resetting our gyro
  ...
  public double[] speedcontrol(double x, double y) {
    ...
    // x and y are target corrdinates
    // getting current x and y coordinates of bot.
    this.a_botXpose = this.pose.getX();
    this.b_botYpose = this.pose.getY();
    
    
    // getting current angle and converting it into 0-360 range
    this.angle = -DriveSubsystem.navx.getAngle() % 360;
    // finding the angle to take but its output  is in range -180 to 180 after passing through atan2 function
    // this is always the ddifference and not the absolute angle
    ...
    // converting angle in the range of 0-360

    this.angletotake = (this.angletotake + 720) % 360;
    // pid.setDeadband

    // finding the distance neend to be travelled to get to that coord
    double d = Math.sqrt(Math.pow((this.a_botXpose - x), 2) + Math.pow((this.b_botYpose - y), 2));

    // finding angle to trvale to get tot that coordinate
    double d_theta = this.angletotake - this.angle;

    // direction in which we need to set out motors to in order to get to that target while spinning
    // for negative theta dir turn clockwise for positive turn clockwise.
